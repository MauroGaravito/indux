...
1. **Admin** crea proyecto y configuracion:
   * Nombre/direccion/map key.
   * Campos personales (`ProjectFields`).
   * Slides (ppt/pdf/img) + thumb.
   * Quiz.
2. **Admin** envia a revision -> `reviewStatus = pending`.
3. **Manager** revisa en `/manager/review`:
   * Ve snapshot de config/fields/slides.
   * Aprueba o declina (con motivo). Aprobado -> `reviewStatus = approved`.
4. **Manager** asigna workers en proyectos aprobados (`/assignments`). Las listas respetan rol/asignacion.
5. **Worker** ingresa al **Induction Wizard** solo en proyectos aprobados y asignados:
   * Paso 1: Seleccion de proyecto (bloqueado si ya tiene submission aprobada).
   * Paso 2: Datos personales (ProjectFields).
   * Paso 3: Slides.
   * Paso 4: Quiz.
   * Paso 5: Firma y envio.
6. Backend aplica unicidad de submission por worker/proyecto:
   * `pending/declined` se sobreescriben en un nuevo envio.
   * `approved` responde 409; el UI bloquea reintentos.
7. **Manager/Admin** revisan submissions en `/manager/review` o `/admin/review`:
   * Datos del worker, uploads, quiz, firma y links de descarga.
   * Aprobar genera PDF de certificado en S3/MinIO + email al worker; decline guarda motivo.
8. **Dashboards** (manager/admin) reflejan asignaciones, review queue, submissions y salud del sistema.
