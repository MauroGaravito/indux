...
import { UserCreateSchema, UserUpdateSchema, UserStatusPatchSchema } from '../utils/validators.js';
...
router.get('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const filter: any = {};
  if (rawStatus && ['pending','approved','disabled'].includes(rawStatus)) {
...
router.post('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const user = await User.create({ email, name, role, password: passwordHash, status: 'approved', emailVerified: true, emailVerifiedAt: new Date() });
...
router.patch('/:id/status', requireAuth, requireRole('admin'), async (req, res) => {
  const parsed = UserStatusPatchSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });
  try {
    const patch: any = { status: parsed.data.status };
    if (parsed.data.status === 'disabled') patch.disabled = true;
    if (parsed.data.status === 'approved') patch.disabled = false;
    const updated = await User.findByIdAndUpdate(req.params.id, patch, { new: true }).select('-password');
    if (!updated) return res.status(404).json({ error: 'User not found' });
    res.json(updated);
  } catch (e: any) {
