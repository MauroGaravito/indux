...
router.post('/modules/:moduleId/submissions', requireAuth, requireRole('worker'), async (req, res) => {
    ...
    if (typeof req.body?.uploads === 'string') {
        try {
            req.body.uploads = JSON.parse(req.body.uploads);
        }
        catch { }
    }
    ...
    if (mod.reviewStatus !== 'approved')
        return res.status(400).json({ error: 'Module not approved' });
...
router.get('/workers/me/submissions', requireAuth, requireRole('worker'), async (req, res) => {
    ...
    const submissions = subs.map((s) => ({
        ...
        projectId: s.projectId ? s.projectId._id : null,
        project: s.projectId
            ? {
                id: s.projectId._id,
...
router.post('/submissions/:id/approve', requireAuth, requireRole('manager', 'admin'), async (req, res) => {
    ...
    if (!user || !project || !mod)
        return res.status(400).json({ error: 'Invalid submission context' });
    if (req.user.role === 'manager') {
