...
async function isManagerOfProject(managerId, projectId) {
    if (!Types.ObjectId.isValid(managerId) || !Types.ObjectId.isValid(projectId))
        return false;
...
router.get('/user/:id', requireAuth, requireRole('admin', 'manager', 'worker'), async (req, res) => {
    ...
    const filterWithExistingProject = (list) => list.filter((entry) => entry?.project && entry.project?._id);
    try {
...
router.get('/project/:id', requireAuth, requireRole('admin', 'manager', 'worker'), async (req, res) => {
    ...
    try {
        if (req.user.role === 'manager') {
            ...
            if (!allowed)
                return res.status(403).json({ error: 'Manager not assigned to this project' });
        }
            ...
            if (!assigned)
                return res.status(403).json({ error: 'Worker not assigned to this project' });
        }
...
router.get('/manager/:id/team', requireAuth, requireRole('admin', 'manager'), async (req, res) => {
    ...
    try {
        ...
        if (!managed.length)
            return res.status(404).json({ error: 'No managed projects found' });
        ...
        const out = workers.map(w => ({
            ...
            name: String(w.user?.name || ''),
            email: String(w.user?.email || ''),
            projectName: String(w.project?.name || ''),
            projectId: String(w.project?._id || w.project),
        }));
        return res.json(out);
    }
    catch (e) {
