 ...
 */
export async function createBrandConfig(req, res) {
    try {
        ...
        if (existing) {
            ...
            if (req.user?.sub)
                existing.createdBy = existing.createdBy || req.user.sub;
            await existing.save();
 ...
 */
export async function updateBrandConfig(req, res) {
    try {
        ...
        if (companyName != null) {
            if (!isNonEmptyString(companyName))
                return res.status(400).json({ error: 'companyName must be non-empty' });
