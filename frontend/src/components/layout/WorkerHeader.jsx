...
export default function WorkerHeader({ onMenuClick }) {
  const { user, logout } = useAuthStore()
  const displayName = user?.name || user?.email || 'Worker'
  const displayEmail = user?.email || ''
  const initials = (user?.name || user?.email || 'WO').split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()
