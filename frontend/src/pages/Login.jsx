...
const schema = z.object({
  email: z.string().trim().min(3).refine(v => v.includes('@'), { message: 'Enter a valid email' }),
...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    if (!ok) setError('root', { message: 'Sign in details are incorrect' })
    else {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={2} maxWidth={400}>
        ...
        <AsyncButton type="submit" variant="contained" onClick={handleSubmit(onSubmit)}>Sign in</AsyncButton>
      </Stack>
    </form>
  )
}
